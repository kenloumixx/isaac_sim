...
class A1_direct_runner(object):
    def __init__(self, physics_dt, render_dt) -> None:
        ...
        # d) ground truth body pose with a fake covariance      -> fake covariance? 뭔가 축들간의 연관관계를 드러내는건가..?
    ...
    def joint_command_callback(self, data):
        ...
        """
        for i in range(3):   
            self._base_command[i] = data.effort[i]
...
def main():
    ...
    ## init_node : tells rospy the name of the node. -> able to communicate with the ROS master
    ## name should be 'base name' without any '/'
